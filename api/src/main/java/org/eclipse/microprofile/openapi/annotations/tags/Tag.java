 ...
 * appear in two contexts.
 * <p>
 * Define tag objects on a method that implements an operation or on a class that contains operations. They will be
 * gathered and stored in the root object of the OpenAPI document.
 * <p>
 * If the tag annotation definition is associated with a method that corresponds to an operation then the tag name will
 * be added to the OpenAPI document and also be added to the operation tags.
 * <p>
 * Reference to a Tag can be created by setting the 'ref' attribute.
 * <p>
 * If the Tag annotation is specified on a class then each operation in the class will inherit the tag unless the
 * operation specifies Tag(s). An operation can specify an empty Tag to not to be associated with any tag(s) inherited
 * from the class.
 * <p>
...
public @interface Tag {
    ...
    /**
     * The name of this tag. The name must be unique and is case sensitive. It is a REQUIRED property unless this is
     * only a reference to a tag instance.
     *

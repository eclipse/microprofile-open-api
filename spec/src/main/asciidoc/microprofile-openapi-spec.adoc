...
*  a standard way to document non-application APIs: this is probably the most significant reason 
for this new SPI, as the number of APIs exposed by frameworks continues to grow and they all need
to be documented in some way.  We certainly don't want RESTful APIs being documented in HTML tables,
proprietary / custom formats, etc - if it's a REST API it should be documented using OpenAPI.
...
`mp.openapi.spi.platform.jmx=${server_install_dir}/oas/jmx/openapi.yaml`
...
There are two new RESTful endpoints that vendors must expose.  The first is to query which platform OpenAPIs
are currently available:

1. `GET openapi/platform`  - returns a list (following the <<Content format>> rules) of available components exposing
OAS3 docs.  The component name used matches the `<component>` string used during registration inside the key `mp.openapi.spi.platform.<component>`.

A sample returning payload can be:
...
2. `GET openapi/platform/<component>` - return the actual OAS3 doc for that component.

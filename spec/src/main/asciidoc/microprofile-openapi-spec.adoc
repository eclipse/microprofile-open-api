...
:authors: Arthur De Magalhaes (Spec Lead), Leo Christy Jesuraj, Eric Wittmann
:email: ademagalhaes@gmail.com, leo.jesuraj@gmail.com, eric.wittmann@gmail.com
:version-label!:
:sectanchors:
:doctype: book
...
=== Content format
The default format of the `/openapi` endpoint is `YAML`, which should result in
a response with a `Content-Type` header with a value of `application/yaml`.

...
Vendors are required to support a mechanism to expose `platform` OpenAPIs to consumers. 
...
=== Registrying a platform OpenAPI
...
This pattern takes advantage of MicroProfile Config to inject the location of the component's OAS3 doc,
...
There are two new RESTful endpoints that vendors must expose.  The first queries which platform OpenAPIs

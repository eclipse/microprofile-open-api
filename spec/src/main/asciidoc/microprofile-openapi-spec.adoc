...
These are RESTful APIs implemented by the vendor framework itself, including MicroProfile Metrics,
MicroProfile Health Check, JMX-REST connectors, OpenID Connect Providers and any other API
the framework may be exposing.  
...
=== Contributing a platform OpenAPI

The components within a vendor's framework will contribute their platform OpenAPI by using the 
following patttern:

...
== Exposing platform OpenAPIs

Vendors may provide support for a mechanism to expose `platform` OpenAPIs to consumers. 
These are RESTful APIs implemented by the vendor framework itself, which may include 
MicroProfile Metrics, MicroProfile Health Check, JMX-REST connectors, OpenID Connect Providers
or any other API the framework may be exposing.  

The exposure of these platform OpenAPIs allows for:

  *  discovery of available platform endpoints
  *  generation of clients to consume the API (OAS3 -> client_language)
  *  a standard way to document non-application APIs
    **  this is probably the most significant reason for this new SPI, as the number of APIs exposed by 
  frameworks continues to grow and they all need to be documented in some way.  We certainly don't want 
  RESTful APIs being documented in HTML tables, proprietary / custom formats, etc - if it's a REST API 
  it should be documented using OpenAPI.


=== Registering a platform OpenAPI

The components within a vendor's framework will register their platform OpenAPI by using the 
following pattern:

`mp.openapi.spi.platform.<component>=<url_to_OAS3>`

or:

`mp.openapi.spi.platform.<component>=<OAS3_yaml>`

Example:  `mp.openapi.spi.platform.jmx=${server_install_dir}/oas/jmx/openapi.yaml`

Please note that this is an `SPI`, which means it is intended to be set only by the vendor framework, not by
user applications.

This pattern takes advantage of MicroProfile Config to inject the location, or content, of the component's OAS3 doc,
which creates dynamic and composed deployments, only exposing the platform APIs that are currently enabled.

For dynamic frameworks (e.g. loading and unloading OSGi bundles), components can be unregistered by setting
their OAS3 path to empty:

`mp.openapi.spi.platform.jmx=`


=== Exposing platform OpenAPIs

There are two RESTful endpoints that vendors must expose if they choose to provide support for this functionality.
The first queries which platform OpenAPIs are currently available:

*  `GET openapi/platform`  
** returns a list (following the <<Content format>> rules) of available components exposing
OAS3 docs.  
** the component name used matches the `<component>` string used during registration inside the key `mp.openapi.spi.platform.<component>`.

**  A sample returning payload:
...
*  `GET openapi/platform/<component>` 
**  returns the actual OAS3 doc for that component (following the <<Content format>> rules).
**  the `servers` element of the OAS3 doc for each component may be omitted, in which case
the vendor framework must insert an appropriate `servers` value.  

These APIs are public and meant to be used by application developers or anyone that needs to interact with these endpoints.
